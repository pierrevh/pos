...
public class UdpPostOffice implements PostOffice {
    ...
    public void sendMessage(String text) {
        try (DatagramSocket clientSocket = new DatagramSocket()) {
            ...
            DatagramPacket message = new DatagramPacket(payload, payload.length, serverInetAddress, port);
            clientSocket.send(message);
        } catch (IOException unrecoverable) {
            throw new RuntimeException(
                    String.format("Unable to send message '%s' by UDP to %s:%d", text, hostName, port),
                    unrecoverable);
        }
    }
}
